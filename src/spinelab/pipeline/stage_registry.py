...
STAGE_SPECS: dict[PipelineStageName, PipelineStageSpec] = {
    ...
    PipelineStageName.MESH: PipelineStageSpec(
        ...
        description="Legacy mesh extraction (visualization only, not in default pipeline).",
    ),
    PipelineStageName.POINT_CLOUD: PipelineStageSpec(
        stage=PipelineStageName.POINT_CLOUD,
        runner=run_point_cloud_stage,
        dependencies=(PipelineStageName.SEGMENTATION,),
        review_surface="measurement",
        produced_artifact_types=("point-cloud-manifest", "point-cloud-data"),
        description="Extract smooth point clouds from segmentation label maps via Surface Nets.",
    ),
    PipelineStageName.LANDMARKS: PipelineStageSpec(
        stage=PipelineStageName.LANDMARKS,
        runner=run_landmarks_stage,
        dependencies=(PipelineStageName.POINT_CLOUD,),
        review_surface="measurement",
